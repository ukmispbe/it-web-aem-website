...
const legacyAvailabilityRequest = (url, countryCode, partNo) => {
    const url = url
...
const legacyPriceRequest = (url, countryCode, partNo) => {
    const url = url
        .replace('{partnumber}', partNo)
        .replace('{countryCode}', countryCode);
...
export async function getAvailability(url, countryCode, partNo) {
    const url = legacyAvailabilityRequest(url, countryCode, partNo);
...
export async function getPrice(url, countryCode, partNo) {
    const url = legacyPriceRequest(url, countryCode, partNo);
    const response = await postData(url, data);
    const json = await response.json();
    return json;
}
...
export async function addToCart(isCommerceApiMigrated, url, partNo, quantity) {
    if(isCommerceApiMigrated) {
        ...
        const url = addToCartUrlRequest(url, partNo, quantity, cartId);
        const response = await postData(url, data);
        const json = await response.json();
        return json;
        ...
        const url = legacyCartUrlRequest(url, partNo, quantity, cartId);
        const response = await postData(url, data);
        const json = await response.json();
        return json;
    }
