...
class SearchContainer extends Component {
	...
	componentDidMount() {
		...
		this.setState({
			isEprocurementUser: isEprocurementUser(),
		});
	}

	parseFacetsFromUrlToArray = () =>
		this.search.mapFacetGroupsToArray(parse(location.search).facet);
	...
	handleHistoryPush = (query) => {
		...
		this.setState({
				...
				selectedFacets: Object.entries(query.facets).length === 0 ?
					{} :
					Object.assign({}, query.facets),
			},
			async () => {
	...
	isSkuList = (category) => {
		const categoryKey = this.findFacetNameProperty(
			this.props.filterMap,
			category
		);
	...
	findFacetNameProperty = (filterMap, searchValue) => {
		...
		const facet = filterMap.find(
			(item) => item.categoryFacetValue === searchValue
		);
	...
	mapCategories = (categories) =>
		...
		.filter(
			(category) =>
			category.count !== 0 &&
			!!this.findFacetNameProperty(this.props.filterMap, category.value)
		)
		.map((category) => {
			return {
				translation: this.findFacetTranslationProperty(
					this.props.filterMap,
					category.value
				),
				name: category.value,
	...
	setStateForActiveCategory = (query) => {
		...
		const categoryName =
			categoryIndex !== -1 ? this.state.categoryTabs[categoryIndex].name : "";
	...
	executeSearchByContentType = (query, rows) => {
		...
		this.search
			...
			.then((res) => this.searchOnSuccess(query, rows, res, false, "success"))
	...
	getFilterMap = (authoredTags, backendFacets) => {
		...
		const category = authoredTags.find(
			(authoredItem) => authoredItem.categoryFacetName === categoryFacetName
		);
		...
		const orderedFacetsWithCount = orderedFacets.map((facet) => {
			const authTag = authoredTags.find(
				(authoredItem) => facet.facetValue === authoredItem.categoryFacetValue
			);
	...
	searchOnSuccess = (query, rows, res, initCategories = false) => {
		...
		if ("activeIndex" in this.state) {
			...
			newState.activeFilterIndex = this.getActiveFilterIndex(
	...
	submitAnalytics = (data) =>
		Analytics.setAnalytics(analyticTypes.search.name, data);

	getActiveFilterIndex = (contentType, filterMap, facets, facetName) => {
		const mappings = searchMapper.mapFacetGroups(
			contentType,
			filterMap,
			facets
		);
	...
	sortHandler = (e) => {
		const sortOption =
			parseInt(e.value) === 1 ?
			parameterValues.sort.mostRelevant :
			parameterValues.sort.mostRecent;
	...
	resetToSavedState = () => {
		this.setState({
			...
			() => {
				this.pushToHistory(
					this.state.savedState.searchParams,
					this.state.savedState.searchParams.facets
				);
			}
		);
	};
		...
		if (this.state.contentTypeSelected) {
			if (this.state.contentTypeSelected.hasOwnProperty("categoryFacetName")) {
				return {
					...
					facetTranslation: this.state.contentTypeSelected
						.categoryFacetTranslation,
				};
	...
	createTabHistoryEntryForCurrentTab = (query) => {
		...
		tabHistoryEntry.contentTypeSelected = Object.assign({},
			this.state.contentTypeSelected
		);
		tabHistoryEntry.selectedFacets = Object.assign({},
	...
	getTabHistoryEntry = (category) => {
		if (
			...
			this.state.tabHistory.hasOwnProperty(`${category}`)
		) {
	...
	handleHideSortFilterClick = () => {
		...
		const savedParamsStringify = JSON.stringify(
			this.state.savedState.searchParams
		);
	...
	resultsProps = () => {
		return {
			...
			current: this.state.pagination && this.state.pagination.current ?
			...
			items: Array.isArray(this.state.results) ?
				this.state.results :
				this.state.results ?
				this.state.results :
				[],
	...
	asideProps = () => {
		return {
			sortFilterIsPristine: this.state.contentType ||
				this.state.keyword !== parameterDefaults.keyword ?
				false :
				true,
			count: this.state.count,
			sortByText: this.state.sort,
			sortByValue: this.state.unappliedFilters && this.state.unappliedFilters.sort ?
	...
	menuProps = () => {
		return {
			showContentTypeMenu: this.isCategoryOnlySelected(
				this.state.category,
				this.state.contentType
			),
			showFacetMenu: !this.isCategoryOnlySelected(
				this.state.category,
				this.state.contentType
			),
	...
	contentTypeMenuProps = () => {
		return {
			items: this.state.filterMap && this.state.filterMap.orderedFacets ?
				this.state.filterMap.orderedFacets :
				[],
		};
	};
	...
	filterTagsProps = () => {
		return {
			...
			selectedFacets: this.state.unappliedFilters &&
	...
	render() {
		...
		return ( <
			SearchComponent isEprocurementUser = {
				this.state.isEprocurementUser
			}
			text = {
			...
			showSortFilterProps = {
				this.showSortFilterProps()
			}
			showSortFilterEvents = {
				this.showSortFilterEvents()
			}
			resultsProps = {
				this.resultsProps()
			}
			...
			contentTypeMenuProps = {
				this.contentTypeMenuProps()
			}
			contentTypeMenuEvents = {
				this.contentTypeMenuEvents()
			}
			facetMenuProps = {
				this.facetMenuProps()
			}
			facetMenuEvents = {
				this.facetMenuEvents()
			}
			subFacetFiltersProps = {
				this.subFacetFiltersProps()
			}
			subFacetFiltersEvents = {
				this.subFacetFiltersEvents()
			}
			filterTagsProps = {
				this.filterTagsProps()
			}
			filterTagsEvents = {
				this.filterTagsEvents()
			}
			/>
		);
	}
