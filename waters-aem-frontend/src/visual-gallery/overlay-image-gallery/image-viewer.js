...
class ImageViewer extends React.Component {
  ...
  touchActionNotSupported = () =>
    typeof CSS != "undefined" && !CSS.supports("touch-action", "none");
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.state.magnified &&
      !prevState.magnified &&
      this.touchActionNotSupported()
    ) {
      this.touchActionPolyfill = Hammer(this.figureRef.current, {
        touchAction: "pan-x",
      });
    } else if (
      !this.state.magnified &&
  ...
  calculateWidth = () => {
    ...
    const figureWidth = this.figureRef.current.getBoundingClientRect().width;
    const imageSrc = this.props.template.src.replace(/{{width}}/gi, imageWidth);
